...
CR .( Loading FPOUT v1.9  10-Nov-05 ... ) CR
...
\ Test if an enhanced REPRESENT (see proposal elsewhere) is
\ present.  If found the usual work-arounds are bypassed and
\ simpler code installed.  Note: this test involves checking
...
: (F3)  ( F: r -- ) ( places -- c-addr u flag )
  ...
  DUP EF#  [CHAR] 0  FILL  \ engineering fix-up
  R@ REPRESENT >R  TO SN#  1- TO EX#
...
: (F3)  ( F: r -- ) ( places -- c-addr u flag )
  ...
  OVER EF#  [CHAR] 0 FILL  \ engineering fix-up
...
: (FD)  ( F: r -- ) ( n factor -- c-addr u )
  TO EF# (F3) IF  EX# (F2) (F4) 1+ (FC) (F8)  THEN ;
...
: (G.)  ( F: r -- ) ( n -- c-addr u )
  >R  (F1) NIP AND  [ -4 1+  5 2 + ] 2LITERAL WITHIN

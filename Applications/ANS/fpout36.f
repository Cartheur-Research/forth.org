...
\   contents are zero then trailing decimal points are not
\   shown. If non-zero (default state) the decimal point
\   is always displayed.
\
\   FECHAR ( -- c-addr )
\
...
\ mode. Compact mode displays all significant digits with
\ non-essential zeros and signs removed. FS. FE. F. G.
\ are displayed in floating-point compact mode.
\
...
CR .( Loading FPOUT 3.6  2011-02-06 ... ) CR

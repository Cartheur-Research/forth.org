...
The manual has been updated to reflect most updates and changes from
the last release of Neon 2.0 to the current version of Yerk 3.6.4.
Yerk runs on all systems up to 7.1 (although system 7 is best). There
are only minor changes to the manual. I'm sure that some things are
...
manual or the application. In the meantime, check the modifications
file to see what has changed and hints.

In addition to the example sources that Kriya issued with Neon are
some examples of my own, just to show how to use some of the
additional classes of Yerk. Check the supplement folder

If people are interested, I have less general classes for color
quickdraw interfaces, MacTCP classes, and a host of others. These were
written for specific applications, but I could make them available to
...
Yerk is 32 bit clean, but does not respond to high level events. I don't see
...
We have used the language on all macs. There is no reason to believe that it
won't work on any mac+ or beyond. Any system >= 6.0 is recommended.

For those of you already familiar and using Yerk, read the 'modifications' file
which will tell you how the current release is different from previous versions.
Also, the glossary contains more Yerk words, so you should also reread it.
